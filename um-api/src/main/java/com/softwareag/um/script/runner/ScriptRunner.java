package com.softwareag.um.script.runner;
/*
...
public class ScriptRunner {
    ...
    private int numberOfScriptsExecuted = 0;

    public static void runScriptsFromFilePath(Connection mConnection, String filePath) throws Exception {
    	runScriptsFromReader(mConnection, new BufferedReader(new FileReader(filePath)));
	}
    
    public static void runScriptsFromFileStream(Connection mConnection, InputStream fileInputStream) throws Exception {
    	runScriptsFromReader(mConnection, new InputStreamReader(fileInputStream));
	}
    
    public static void runScriptsFromReader(Connection mConnection, Reader reader) throws Exception {

		System.out.println("Executing scripts in database...");
		
    	try {
	    	ScriptRunner runner = new ScriptRunner(mConnection, false, false);
	    	runner.runScript(reader);
    	} catch (SQLException e) {
    	    System.err.println("Unable to execute scripts: " + e);
    	    throw e;
    	}
		System.out.println("Scripts executed succesfully in database");
	}
    
    public static void callStoredProcedure(Connection mConnection, String sql, String... args) throws Exception {

		System.out.println("Calling stored procedure in database... " + sql + " with arguments " + Arrays.toString(args));
		
        String mSql = "{ " + sql + " }";
    	try (CallableStatement stmt = mConnection.prepareCall(mSql)){
            int idx = 1;
            if(null != args) {
	            for(String arg : args) {
    	    ...
    	    System.err.println("Unable to call stored procedure: " + e);
    	    throw e;
		}
		System.out.println("Stored procedure called successfully.");
	}
    
    public static void runScripts(Connection mConnection, String sql, String... args) throws Exception {

		System.out.println("Executing scripts in database...");
		
    	try {
	    	ScriptRunner runner = new ScriptRunner(mConnection, false, false);
	    	runner.runScript(mConnection, sql, args);
    	} catch (SQLException e) {
    	    System.err.println("Unable to execute scripts: " + e);
    	    throw e;
    	}
    ...
    public void runScript(Connection conn, String sql, String... args) throws Exception {

		System.out.println("Running SQL in database... " + sql + " with arguments " + Arrays.toString(args));
    	...
    	try (PreparedStatement statement = conn.prepareStatement(sql)){
	        ...
	        try {
	            ...
	            final String errText = String.format("Error executing '%s': %s",
	                    sql, e.getMessage());
    ...
    public ScriptRunner(Connection connection, boolean autoCommit,
        ...
        this.numberOfScriptsExecuted = 0;
        File logFile = new File("logs/create_db_" + DatabaseMigrator.CURRENT_TIME_MILLIS + ".log");
        File errorLogFile = new File("logs/create_db_error_" + DatabaseMigrator.CURRENT_TIME_MILLIS + ".log");
        try {
            if (logFile.exists()) {
    ...
    public void runScript(Reader reader) throws IOException, SQLException {
        try {
    		...
    		System.out.println(" [Total scripts executed - " + numberOfScriptsExecuted + "]");
    ...
    private void execSqlCommand(Connection conn, StringBuffer command,
		...
		if(numberOfScriptsExecuted%50 == 0)
